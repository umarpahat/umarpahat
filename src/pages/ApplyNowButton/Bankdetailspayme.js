...
const Bankdetalspayme = (props) => {
  ...
  async function getSignedUrl() {
    const pathArray = [`bank_statement/${props.user.id}/0.pdf`, `bank_statement/${props.user.id}/1.pdf`, `bank_statement/${props.user.id}/2.pdf`]
  ...
  async function updateBankDetails() {
    const payload = {account_number: actNumber, bank_address: branchName ,bank_name: bankName,
      ifsc_code: ifsc,
      }
    return await api.post('/api/user_details/bank_details/', payload, {headers: { 'Authorization': 'Token ' + props.token } })
  }
...
if (!actNumber) {seterrorAct("Please enter account number"); return;}
if (!ConfrmActNumber) {seterrorConfAct("Please enter confirm account number"); return;}
if (ConfrmActNumber !== actNumber) {seterrorConfAct("Account number and Confirm account number does not matched."); return;}
if (!ifsc) {setifscErro("Please enter ifsc code"); return;}
if (!bankName) {seterrorbankName("Please enter bank name"); return;}
if (!branchName) {seterrorbranchName("Please enter branch name"); return;}
if (!bankStatementObj.length) {seterrorBnakStatement("Please upload bank statement"); return;}
setloader(true)
let updatedocStatus = []
const promiseTest = bankStatementObj.map((value, index) => {
  updatedocStatus.push(updateDocStatus({docType:"bank_statement", path: `bank_statement/${props.user.id}/${index}.pdf`}))
 ...
 Promise.all([...promiseTest, ...updatedocStatus, ...[updateBankDetails()]]).then((response)=>{
  ...
  const content = bankStatementObj.map((value, index) => (
    ...
    <div className="ml-3 w-100">
      <div className="d-flex justify-content-between">
        ...
        <img src={Cancelicon} onClick={()=>handleRemoveBankObj(index)} alt="bank"/>
      </div>
    </div>
  </div>
  ));
  ...
  const handleBankUpload = (event) => {
    ...
    setbankStatementObj(bankStatementArr => [...bankStatementArr, ...[event.target.files[0]]])
  }
              ...
              <div className="form-block">
                <div class="form-group ms-input-group">
                  ...
                  <input
                    ...
                    value={actNumber}
                    onChange={(event)=>{seterrorAct(""); setactNumber(event.target.value)}}
                  />
                  {errorAct ? <span style={{color:"red"}}>{errorAct}</span> : null}
                </div>
                <div class="form-group ms-input-group">
                  ...
                  <input
                    ...
                    onChange={(event)=>{seterrorConfAct("");setConfrmActNumber(event.target.value)}}
                  />
                  {errorConfAct ? <span style={{color:"red"}}>{errorConfAct}</span> : null}
                </div>
                <div class="form-group ms-input-group">
                  ...
                  <input
                    ...
                    value={ifsc}
                    onChange={(event)=>{setifscErro(""); setifsc(event.target.value)}}
                  />
                  {ifscError ? <span style={{color:"red"}}>{ifscError}</span> : null}
                </div>
                <div className="row">
                  <div class="form-group ms-input-group col-6">
                    ...
                    <input
                      ...
                      placeholder="Delhi"
                      value={branchName}
                      onChange={(event)=>{seterrorbranchName(""); setbranchName(event.target.value)}}
                    />
                    {errorbranchName ? <span style={{color:"red"}}>{errorbranchName}</span> : null}
                  </div>
                  <div class="form-group ms-input-group col-6">
                    ...
                    <input
                      ...
                      value={bankName}
                      onChange={(event)=>{seterrorbankName(""); setbankName(event.target.value)}}
                    />
              ...
              <div>
                ...
                <div class="form-group ms-input-group">
                  ...
                  <input
                    ...
                    onChange={(event) => {setbankStatementPassword(event.target.value)}}
                  />
                </div>
              </div>
              <input
...
export default connect(mapStateToProps, dispatchToProps)(Bankdetalspayme)
